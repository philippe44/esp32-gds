...
extern const uint8_t image_jpg_start[]   asm("_binary_image_stella_jpg_start");
extern const uint8_t image_jpg_end[]     asm("_binary_image_stella_jpg_end");
...
esp_err_t decode_image(uint16_t ***pixels) 
{
    ...
    *pixels=calloc(IMAGE_H, sizeof(uint16_t*));
    ...
    for (int i=0; i<IMAGE_H; i++) {
        (*pixels)[i]=calloc(1,IMAGE_W*sizeof(uint16_t));
        if ((*pixels)[i]==NULL) {
